import {useHttpServerConfigStore} from '../stores/httpServerConfig'
 ...
 */
const ipcApiRoute = {
  quitApp: 'controller.window.quit',
  closeWindow: 'controller.window.close',
  minimizeWindow: 'controller.window.minimize',
 ...
 * 访问内置http服务
 */
const requestHttp = (uri : string, ...parameter :any[]) => {
  // url转换
  const storage = useHttpServerConfigStore();
  const host = storage.config.server || 'http://127.0.0.1:7071';
